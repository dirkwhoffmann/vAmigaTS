...
`bplam4.retrosh` runs under `A1200_2MB`, the AGA configuration scheme. It is the first script under Agnus/AGA: before that scheme existed the tests here were photo-only, and the numbers above had to be produced by setting the Agnus and Denise revisions by hand. The rest of Agnus/AGA could be scripted the same way now.
