...
;
;
; bplam5.s -- sprite clipping at the two edges of the display window.
;
; bplam4 with two Pacman sprite columns added. Everything about the
; bitplanes, the palette and the window sweep is unchanged: three sections
; (lores, hires, super hires), each divided into six subsections that place
; DIWSTRT before, on and after the first bitplane pixel, first with the
; border open and then with BRDRBLNK set.
;
;
; WHAT IT ADDS
; ------------
;
; Two columns of Pacmen run the full height of the picture:
;
;   sprite 0    lores $73, so it sits exactly where the window opens in the
;               super hires section and is crossed by the lores and hires
;               sweep at $77 / $7F / $87.
;
;   sprite 1    lores $1B9, straddling DIWSTOP at $1C1 with eight pixels
;               inside the window and eight outside it.
;
; A sprite is gated by the horizontal display window like a bitplane is, so
; the left column has to be cut back further in every subsection as DIWSTRT
; moves right, and the right column has to lose its outer half at DIWSTOP
; in every one of them.
;
; The interesting part is that the two gates are NOT the same. Bitplanes
; also wait for the first BPL1DAT write of the line, which is why bplam4's
; "before" and "exact" subsections have the same left edge -- DIWSTRT to the
; left of the data does not open the window any earlier. Sprites are armed
; by that same write about one lores pixel earlier, so the two columns
; answer a question the bitplanes cannot:
;
;   left column cut identically in "before" and "exact"
;                       the sprite gate follows the bitplane gate
;
;   left column visibly wider in "before" than in "exact"
;                       the sprite gate follows DIWSTRT alone
;
; The right column is the control. DIWSTOP never moves, so every one of the
; eighteen subsections must clip it in exactly the same column, whatever the
; resolution and whatever BRDRBLNK is doing. A right column that changes
; width between subsections means something other than DIWSTOP is closing
; the window.
;
;
; NOTES
; -----
;
; BPLAM is $20 here rather than bplam4's $10. Sprites 0 and 1 take their
; colours from registers 17 to 19, and with BPLAM $10 the bitplane data
; would be painted out of registers 16 to 31 as well -- the sprites would be
; drawn in playfield colours. $20 moves the playfield to registers 32 to 47
; and leaves the sprite palette alone.
;
; FMODE is $0001: 32 bit bitplane fetches, which four planes in super hires
; need, but bits 3 and 2 clear, so the sprites stay 16 pixels wide.
;
; BPLCON2 is $0024, which puts both playfields behind all sprites. The
; columns have to be drawn over the bitplane data, not under it.
;
; The sprite lists are built at run time rather than assembled, because the
; picture is 258 lines tall and the vertical position in a sprite control
; word is nine bits, the ninth being held in SPRxCTL. Fifteen copies of the
; same sixteen line Pacman are emitted per column with the control words
; computed from the line number.
;
...
SPR_PERIOD          equ 18            ; 16 lines of Pacman plus a gap
SPR_REPS            equ 15            ; enough to reach the bottom
...
.colorLoLoop:
	...
	; they are overwritten here with three colours that stand out against
	; the bitplane data. Both nibbles again, LOCT=0 then LOCT=1.
	move.w  #$0000,BPLCON3(a1)      ; bank 0, LOCT=0
