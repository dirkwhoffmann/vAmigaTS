...
;
;
; bplam8.s -- bplam7 with every second Pacman left out.
;
; bplam7 answers what the window does to a sprite. It cannot answer what the
; window does WITHOUT one, because in bplam7 there is a sprite on very nearly
; every line: sixteen lines of Pacman, a two line gap, sixteen more. Two lines
; of gap are not enough to read anything off, and they are the same two lines
; in every repeat, so anything the sprite machinery leaves behind is present
; almost everywhere and there is nothing clean to compare it against.
;
; So here the period doubles and the count halves. Every second Pacman is
...
;
; Twenty lines is a band, not a gap. It can be measured, averaged, and read
; at a glance, and it sits directly above and below a band that has a sprite
; in it at the same horizontal position. Anything that differs between the
; two bands at the clipping edge is the sprite machinery's doing and nothing
; else -- same bitplanes, same palette, same window sweep, same line of the
; sweep, only the sprite present or absent.
;
; This is the test for the border artifact. The sprite gate opens one lores
; pixel before the bitplane gate (SPRITE_LATENCY = BPLDAT_LATENCY - 4), and
; in that gap a transparent sprite pixel is drawn in the background colour
; where it should be showing border. In a sprite band those columns take the
; picture's blue; in a no-sprite band the same columns must take the border
; colour. Two vertically adjacent bands, one column apart in nothing else,
; is the cleanest statement of that difference this suite can make.
;
; The Pacmen that remain are on the same lines as bplam7's even numbered
; ones -- SPR_TOP is unchanged and 36 is a multiple of 18 -- so bplam7 and
; bplam8 can be laid on top of each other row for row, and every band of
; bplam8 that has a sprite in it has the identical sprite in bplam7.
;
; Everything else is bplam7: the body is painted in colour[BPLAM] so it has
; no edge against the picture, and the eyes keep their own colours as the
; only proof the sprites are still being drawn. The same caveat applies --
; the eyes sit at sprite pixels 6 to 8, near the left of the figure, which
; is the part the window clips first, so in the lores and hires sections the
; left column has no visible landmark. That is not a defect here: in this
; test the sprite band is identified by the bands around it, not by anything
; inside it.
;
...
.colorLoLoop:
	...
	; The body takes colour[BPLAM], which is what the playfield draws
	; wherever a plane is clear, so it has no edge against the picture.
	; It is computed rather than written out, so that it matches the
	; palette exactly, low nibble included.
	move.w  #$0000,BPLCON3(a1)      ; bank 0, LOCT=0
	moveq   #BPLAM,d5
	bsr     .makeColor              ; d5 -> d2
	move.w  d2,COLOR17(a1)          ; body: the same blue as index 0
	move.w  #$0F80,COLOR18(a1)      ; eyes
	move.w  #$0FFF,COLOR19(a1)      ; eyes
	move.w  #$0200,BPLCON3(a1)      ; bank 0, LOCT=1
	moveq   #BPLAM,d5
	bsr     .makeColor
	move.w  d2,COLOR17(a1)
