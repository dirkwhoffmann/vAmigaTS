...
;
;
; bplam7.s -- bplam5 with the Pacman body painted out.
;
; Everything is bplam5: the same bitplanes, the same palette, the same
; window sweep, the same two sprite columns in the same places. One thing
; differs, and it is a colour.
;
; In bplam5 the Pacman body is yellow against a blue picture. That is easy
; to see, which is the point, but it is also the worst case for reading the
; position where the window clips the column: a hard yellow-to-blue edge is
; exactly what a real monitor smears, and the smear is wider than the effect
; being measured. Every reading of that edge inherits the bleeding.
;
; Here COLOR17, the body, is set to the colour the playfield draws its index
; 0 pixels in -- colour[BPLAM] -- so the body has no edge against the
; picture at all. It is computed with the same .makeColor the palette loop
; uses rather than written as a literal, so it matches to the low nibble.
;
; The eyes are left alone. They are drawn in colours 2 and 3, COLOR18 and
; COLOR19, they are a few pixels wide, and they are the only thing that
; still marks where the sprite is. A column of eyes with no body around it
; proves the sprites are being drawn and shows where they sit, without
; putting a high contrast edge anywhere near the measurement.
;
; Read bplam5 and bplam7 together. bplam5 says how much of the sprite
; survives; bplam7 says the same thing without the bleeding, and the two
; should agree.
;
; One thing to expect: the playfield is a fine comb of index 0 and index 1,
; so the body vanishes against the index 0 stripes and remains faintly
; visible against the index 1 ones. It disappears where it matters, which is
; at the clipping edge and against the border.
;
