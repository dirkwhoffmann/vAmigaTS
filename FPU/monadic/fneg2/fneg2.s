    include "../../fpureg.i"
	include "../../arith2.i"

trap0:
    fmove   #$40,FPCR   ; Precision: Single, Rounding: To nearest
    TEST    fneg
    rte

info: 
    dc.b    'FNEG2', 0
    even 

expected:
    dc.b    $C0,$00,$00,$00,  $C9,$0F,$DB,$00  ; 1
    dc.b    $00,$00,$00,$00,  $08,$00,$00,$08  ; 2
    dc.b    $C0,$00,$00,$00,  $AD,$F8,$54,$00  ; 3
    dc.b    $00,$00,$00,$00,  $08,$00,$00,$08  ; 4
    dc.b    $BF,$FF,$00,$00,  $B8,$AA,$3B,$00  ; 5
    dc.b    $00,$00,$00,$00,  $08,$00,$00,$08  ; 6
    dc.b    $BF,$FD,$00,$00,  $DE,$5B,$D9,$00  ; 7
    dc.b    $00,$00,$00,$00,  $08,$00,$00,$08  ; 8
    dc.b    $BF,$FE,$00,$00,  $B1,$72,$18,$00  ; 9
    dc.b    $00,$00,$00,$00,  $08,$00,$00,$08  ; 10
    dc.b    $C0,$00,$00,$00,  $93,$5D,$8E,$00  ; 11
    dc.b    $00,$00,$00,$00,  $08,$00,$00,$08  ; 12
    dc.b    $FF,$FF,$00,$00,  $00,$00,$00,$00  ; 13
    dc.b    $00,$00,$00,$00,  $0A,$00,$00,$08  ; 14
    dc.b    $7F,$FF,$00,$00,  $00,$00,$00,$00  ; 15
    dc.b    $00,$00,$00,$00,  $02,$00,$00,$08  ; 16
    dc.b    $7F,$FF,$00,$00,  $40,$00,$00,$00  ; 17
    dc.b    $00,$00,$00,$01,  $01,$00,$00,$88  ; 18
    dc.b    $7F,$FF,$00,$00,  $40,$00,$00,$00  ; 19
    dc.b    $00,$00,$00,$00,  $01,$00,$00,$88  ; 20
    dc.b    $FF,$FF,$00,$00,  $40,$00,$00,$00  ; 21
    dc.b    $00,$00,$00,$01,  $09,$00,$00,$88  ; 22
    dc.b    $FF,$FF,$00,$00,  $40,$00,$00,$00  ; 23
    dc.b    $00,$00,$00,$01,  $09,$00,$00,$88  ; 24
