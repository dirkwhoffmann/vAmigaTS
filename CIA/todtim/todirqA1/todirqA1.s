...
MAIN:	
	...
	; Configure CIAs
	move.b  #$84,CIAA_ICR       ; Enable TOD interrupts
 
	; Initialize variables / counters
	moveq   #0,d0               ; Visualized value
	moveq   #0,d3               ;
	moveq   #0,d4               ;
	moveq   #0,d5               ;
	moveq   #0,d6               ;
...
irq2:
	; Read current beam position (d0 = xxxxxxxx00000000, d3 = 00000000xxxxxxxx)
	move.w  VHPOSR(a1),d0
	move.b  d0,d3
	and.w   #$FF00,d0

	; Compute average of low byte (in d6)
	add.l   d3,d4 
	addq    #1,d5
	move.l  d4,d6
	divu    d5,d6
	...
	move.b  CIAA_ICR,d6                 ; Acknowledge (CIA)
