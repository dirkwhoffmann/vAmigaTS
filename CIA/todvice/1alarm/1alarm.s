...
irq2: ; CIA A
	move.w  VPOSR(a1),d5
	move.b  CIAA_ICR,d0                 ; Acknowledge (CIA)
...
irq6: ; CIA B
	...
	; move.w  #$00F,COLOR00(a1)
	addq    #1,d4                       ; Count alarms
	rte

irq3:
	move.w	#$0020,INTREQ(a1)	        ; Acknowledge

	subq    #1,counter                  ; Only proceed every second
	bne     .exit 
	move.w  #25,counter

	jsr     time0alarm0                 ; Trigger TOD alarm
	addq    #1,d3                       ; Count passes

	; Compare passes and alarms
	lea	    spare(pc),a0
	add     #2,a0
	move.w  #$0F0,(a0)
	move.w  d4,d6
	sub.w   d3,d6
	sub.w   d3,d6   ; d4 must be twice the value of d3, because of the "TOD bug"
	beq.s   .skip
	move.w  #$F00,(a0)
.skip:
